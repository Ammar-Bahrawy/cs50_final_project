...
function positionControls(controls) {
  for (let i = 0; i < 6; i++) {
    controls[i].position.set(30 + i * 4, 30, 0);
  }
}

function updateCircleControls(controls) {
  for (let i = 0, l = controls.length; i < l; i++) {
    controls[i].update;
  }
}

function initMoreCameras() {
  const numCameras = 1;
  ...
  for (let i = 0; i < numCameras; i++) {
    ...
    camera.position.set(41.5, 30, 10);
    camera.zoom = 2;
    camera.lookAt(41.5, 30, 0);
    cameras.push(camera);
  }
  return cameras;
}

function resize(camera, renderer, camera2) {
  camera.aspect = window.innerWidth / (window.innerHeight * 0.7);
  camera.updateProjectionMatrix();

  camera2.aspect = window.innerWidth / (window.innerHeight * 0.3);
  camera2.updateProjectionMatrix();

  renderer.setSize(window.innerWidth, window.innerHeight);
}

function splitRender(renderer, scene, camera, camera2) {
  let canvasHeight,
    canvasWidth,
    mainViewportHeight,
    mainViewportWidth,
  ...
  mainViewportWidth = canvasWidth; // 70% width for main screen
  mainViewportHeight = canvasHeight * 0.7;
  smallViewportHeight = canvasHeight - mainViewportHeight;
  ...
  renderer.setScissor(0, 0, mainViewportWidth, smallViewportHeight);
  renderer.setViewport(0, 0, mainViewportWidth, smallViewportHeight);
  renderer.render(scene, camera2);
  // ******************************
  // renderer.setScissor(0, SCREEN_HEIGHT / 2, SCREEN_WIDTH, SCREEN_HEIGHT / 2);
